...
5. Run the real import with a durable state file:

   `tenant-migrator --state migration.sqlite teams-channel --team-id TARGET_TEAM --channel-id TARGET_CHANNEL --messages messages.json`

   `tenant-migrator --state migration.sqlite onedrive --source-user SOURCE_USER_ID --target-user TARGET_USER_ID`
...
Create `user-map.json` as a JSON object such as `{"SOURCE_USER_ID": "TARGET_USER_ID"}`, then dry-run and import in the target tenant:
   ...
   `tenant-migrator --state chat-pilot.sqlite teams-chat --bundle chat-bundle.json --user-map user-map.json`
...
If a failed run leaves a target chat in migration mode, complete that session before retrying: `tenant-migrator complete-chat --chat-id TARGET_CHAT_ID`. Then use a fresh state database for the retry.

## Batch migration

Use [migration.example.yaml](migration.example.yaml) as a starting point. The master [mapping.example.json](mapping.example.json) defines the users in scope and their source/target IDs. Workload mapping files then reference those stable user keys. The batch command processes Teams chats sequentially, writes bundles for audit/retry, validates all mappings before import, continues after an individual failure, and writes a report. Exchange, OneDrive, and SharePoint mappings are also accepted for planning and validation; only Teams chat execution is currently wired into this batch command.
...
Rerunning the same command skips chats recorded as completed. Failed chats can be corrected in the mapping/configuration and rerun; preserve the state database, bundles, and report together.
...
The state database is the resume ledger. Keep it backed up with the extracted payloads, and do not reuse it for a different migration.
