...
class StateStore:
    ...
    def __init__(self, path: str | Path, batch_size: int = 1):
        ...
        self.connection.execute(
            "CREATE TABLE IF NOT EXISTS checkpoints (workload TEXT NOT NULL, source_id TEXT NOT NULL, target_id TEXT, status TEXT NOT NULL, detail TEXT, user_key TEXT, permission_status TEXT, teams_type TEXT, PRIMARY KEY (workload, source_id))"
        )
        existing_columns = {row[1] for row in self.connection.execute("PRAGMA table_info(checkpoints)").fetchall()}
        for column in ("user_key", "permission_status", "teams_type"):
            if column not in existing_columns:
                self.connection.execute(f"ALTER TABLE checkpoints ADD COLUMN {column} TEXT")
        self.connection.commit()
        self._lock = threading.Lock()
        self._batch_size = max(1, batch_size)
        # (target_id, status, detail, user_key, permission_status, teams_type)
        self._pending: dict[tuple[str, str], tuple[str | None, str, str | None, str | None, str | None, str | None]] = {}
    ...
    def teams_type_for(self, workload: str, source_id: str) -> str | None:
        with self._lock:
            pending = self._pending.get((workload, source_id))
            if pending is not None:
                return pending[5]
            row = self.connection.execute(
                "SELECT teams_type FROM checkpoints WHERE workload = ? AND source_id = ?",
                (workload, source_id),
            ).fetchone()
            return row[0] if row else None
    ...
    def mark(
        ...
        with self._lock:
            ...
            if teams_type is None and existing is not None:
                teams_type = existing[5]
            self._pending[(workload, source_id)] = (target_id, status, detail, user_key, permission_status, teams_type)
    ...
    def _flush_locked(self) -> None:
        if not self._pending:
            return
        rows = [
            (workload, source_id, target_id, status, detail, user_key, permission_status, teams_type)
            for (workload, source_id), (target_id, status, detail, user_key, permission_status, teams_type) in self._pending.items()
        ]
        self.connection.executemany(
            "INSERT INTO checkpoints(workload, source_id, target_id, status, detail, user_key, permission_status, teams_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?) "
            ...
            "teams_type=COALESCE(excluded.teams_type, checkpoints.teams_type)",
            rows,
        )
        self.connection.commit()
        self._pending.clear()
